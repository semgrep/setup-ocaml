...
// Pinned to opam 2.5.0 in this fork; see commit "pin opam". Upstream uses
// "<2.6.0" here, which excludes 2.6.x pre-releases that may contain breaking
// changes to the CLI or repository format.
const OPAM_STABLE_VERSION_RANGE = ">=2.5.0 < 2.5.1";
  ...
  await core.group("Initialising opam state", async () => {
    ...
    if (PLATFORM === "windows") {
      ...
      if (WINDOWS_ENVIRONMENT === "cygwin") {
        ...
        extraOptions.push(`--cygwin-extra-packages=${CYGWIN_EXTRA_PACKAGES.join(",")}`);
      }
    }
